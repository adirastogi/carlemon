...
def calc_splitinfo_feature(training_data,total,values_dict,feature):
	split_info = 0.0
	for value in values_dict:
		conditioned_total = float(values_dict[value][0] + values_dict[value][1])
		term1 = float(conditioned_total)/float(total)
		if term1 == 1.0:
			continue
		term2 = float(math.log(term1,2))
		complete_term = float(term1)*float(term2)
		split_info = float(split_info) + float(complete_term)
	if split_info == 0:
		split_info = -1
	return (-1)*split_info


def calc_gain_ratio_feature(total,training_data,feature_name,total_entropy_data):
	...
	split_info = calc_splitinfo_feature(training_data,total,values_dict,feature_name)
	gain_ratio = float(gain)/float(split_info)
	return gain_ratio,values_dict
...
def run_dt(training_data,features,depth):
	...
	max_gain_ratio = (-1)*sys.maxint
	split_feature  = None
	split_feature_values = None
	num_examples = len(training_data)

	for feature in features:
		gain_ratio,values_dict = calc_gain_ratio_feature(num_examples,training_data,feature,total_entropy_data)
		#print "\t\t"*depth, "gain_ratio of feature\t",feature,"\tgain=\t",gain
